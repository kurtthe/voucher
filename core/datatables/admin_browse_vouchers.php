...
if(isset($requestData['order']) && is_array($requestData['order'])){
    ...
    foreach($requestData['order'] as $order){
        ...
        }else if($requestData['columns'][$order['column']]["data"] == 'max_redemptions'){

                //$orderArray["Job.business_name"] =    strtoupper($order["dir"]);
                $orderBy .= " max_redemptions ".strtoupper($order["dir"]);

        }else if($requestData['columns'][$order['column']]["data"] == 'voucher_order'){

                //$orderArray["Job.business_name"] =    strtoupper($order["dir"]);
                $orderBy .= " voucher_order ".strtoupper($order["dir"]);
...
//    }
    ...
    $editLink = '<a href="edit_voucher.php?id='.$outlet_id.'&item_id='.$voucher->id.'" title="Edit" class="btn btn-info btn-sm"><i class="fa fa-pencil"></i></a>';
